...
export default function Landing() {
  ...
  const cursorHeatmap = useRef<{ x: number; y: number; t: number }[]>([]);
  ...
  useEffect(() => {
    const onClickCapture = (e: MouseEvent) => {
      ...
      if (!el) return;
      let key = el.getAttribute("data-track");
      if (!key) {
        if (el instanceof HTMLButtonElement) {
          key =
            el.type === "submit"
              ? "submit"
              : el.textContent?.trim().slice(0, 80) || "button";
        } else if (el instanceof HTMLAnchorElement) {
          key = `a:${el.getAttribute("href") || "link"}`;
        } else {
          key = "input";
        }
      }
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      const now = Date.now();
      const sec = Math.floor((now - pageT0.current) / 1000);
      const { x, y } = lastCursor.current;
      cursorHeatmap.current.push({ x, y, t: sec });
      if (cursorHeatmap.current.length > HEAT_CAP) {
        cursorHeatmap.current.splice(0, cursorHeatmap.current.length - HEAT_CAP);
      }
      postLeadBehaviorPing({
        application_id: 0,
        time_on_page: sec,
        buttons_clicked: JSON.stringify(clickMap.current),
        cursor_positions: JSON.stringify(cursorHeatmap.current),
        return_frequency: 0,
      });
    }, 1000);
  ...
  const svcList = adminRow?.services ?? [];
  const productInterestIdx = svcList.findIndex((s) => {
    const t = typeof s.title === "string" ? s.title : "";
    const sid = typeof s.id === "string" ? s.id : "";
    if (t !== "" && t === form.product_interest) return true;
    if (t === "" && sid !== "" && sid === form.product_interest) return true;
    return false;
  });
  const productInterestSelectValue =
    svcList.length > 0
      ? productInterestIdx >= 0
        ? String(productInterestIdx)
        : "__custom__"
      : "";
    ...
    <div className="page">
      ...
      <div className="shell">
        <header className="hero">
          ...
          <p>
            {" "}
            Заявка для тёплых клиентов: заполните поля — мы подберём формат сотрудничества и
            свяжемся с вами.{" "}
          </p>
          <p className="admin-landing-link">
            <Link to="/admin" data-track="admin_link">
              Админ-панель
            </Link>
          </p>
        </header>
            ...
            <div className="field-full">
              <label>
                ...
                {svcList.length > 0 ? (
                  <>
                    <select
                      value={productInterestSelectValue}
                      onChange={(e) => {
                        const v = e.target.value;
                        if (v === "__custom__") {
                          onChange("product_interest", "");
                          return;
                        }
                        const i = Number(v);
                        const s = svcList[i];
                    ...
                    {productInterestSelectValue === "__custom__" ? (
                      <input
                        className="input-follow-select"
                        value={form.product_interest}
                        onChange={(e) => onChange("product_interest", e.target.value)}
                        placeholder="Опишите продукт или услугу"
                      />
                    ) : null}
                  </>
                ) : (
